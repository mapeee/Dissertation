...
f = open(path, mode='r')
for i in f: path = i
path = Path.joinpath(Path(r'C:'+path),'Dissertation','VarK_MIV.txt')
f = path.read_text()
f = f.split('\n')
...
for Bezug in Resolution:
    ...
    for groups in f:
        if "Luftlinie_NMIV" not in groups:continue
        gr = f[groups]
        r = 1
        print groups
        for i in gr:
            ...
            a100 = pandas.DataFrame(np.array(gr_v[i]))
            a100.loc[:,"n"] = 1
            data_v = pandas.DataFrame(np.array(gr[i.replace("100",Bezug[1])])) ##name Vergleichssystem     
            data_100 = pandas.DataFrame(np.array(gr[i]))
            ws.write(r, 0, len(np.unique(a100["Ziel_ID"]))) #Only in the first column                            
            ...
            data_v["Meter"] = data_v["Meter"]/1.2
            data_v["Meter"] = data_v["Meter"]*1.38
            data_v["tFuss"]  = data_v["Meter"]/66.6666 ##4.0 km/h
            data_v["tRad"]  = data_v["Meter"]/216.6666 ##14.2 km/h
            
#            print i.split("_")[1]
#            print round((a100[a100["tFuss"]<=60].Meter.mean()) / ((data_100[data_100["tFuss"]<=60].Meter.mean())/1.2),2)
            ...
            if "_NMIV" in i: ##tFuss
                ...
                ##--export--##
                if export == 1: 
                    if "E_Hausarzt_NMIV_100" == i and export_bezug == Bezug[0]: zbezug.to_excel("U:"+f[5]+export_bezug.split("_")[1]+"_VarK_"+zent+met+"_NMIV.xls")
            ...
            if "_NMIV" in i: ##tRad   
                ...
                ##--export--##
                if export == 1: 
                    if "E_Hausarzt_NMIV_100" == i and export_bezug == Bezug[0]: zbezug.to_excel("U:"+f[6]+export_bezug.split("_")[1]+"_VarK_"+zent+met+"_NMIV.xls")
