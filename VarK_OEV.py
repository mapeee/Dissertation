...
f = open(path, mode='r')
for i in f: path = i
path = Path.joinpath(Path(r'C:'+path),'Dissertation','VarK_OEV.txt')
f = path.read_text()
f = f.split('\n')
...
##--Path--##
Datenbank = "C:\\Geodaten\\Diss_2019.h5"
#Datenbank_anb = "C:\\Geodaten\\MRH_OEV_2019.h5"
f = h5py.File(Datenbank,'r+') ##HDF5-File
#f_anb = h5py.File(Datenbank_anb,'r+') ##HDF5-File
#gr_anbindung = f_anb["Anbindungen"]
wb = xlwt.Workbook()
modi = ["OEV_ohne","OEV_Luft","OEV_1"]
if gew == 0:
...
for Bezug in Resolution:
    ...
    for modus in modi:
        ...
        spalte+=11
    ws.write(0, 35, "Anteil T-z-T")
    ws.write(0, 36, "Anteil 100") 
          
    for groups in f:
        if "Ergebnisse_OEV" in groups:
            gr = f[groups]
            r = 1
            print groups
            for i in gr:
                ...
                if "_OEV" in i:
                    data_v["Reisezeit"] = (data_v["Reisezeit"]-data_v["Anbindungszeit"])-data_v["Abgangszeit"]

                    ws.write(r, 35, round(float(len(data_v[data_v["Anbindungszeit"]<=17]))/len(data_v),3))
                   
                    
#                    anb = gr_anbindung["A_"+i.split("_")[1]]
#                    anb = pandas.DataFrame(np.array(anb))
#                    anb["RANK"] = anb.groupby(["Start_ID","Bahn"])["tFuss"].rank(method="first", ascending=True)
                    
                    if meter == 1:continue
                    w = "Reisezeit"
                    z100 = a100[a100["Reisezeit"]<=60]
                    ws.write(r, 36, round(float(len(z100[z100["Anbindungszeit"]<=17]))/len(z100),3))
                    
                    z100["Reisezeit"] = (z100["Reisezeit"]-z100["Anbindungszeit"])-z100["Abgangszeit"]                    
                    ...
                    ##--export--##
                    if export == 1: 
                        if "E_Hausarzt_OEV_100" == i and export_bezug == Bezug[0]: zbezug.to_excel("U:"+f[2]+export_bezug.split("_")[1]+"_VarK_"+zent+"_OEV.xls")
        ...
        if "Anbindung_OEV" in groups:
            gr_anb = f[groups]
            r = 1
            print "Luft"
            for i in gr_anb:
                if "100" not in i:continue
                if "100_1" in i: continue
                if "meter" in i:continue
                if "PuR_" in i or "Hst_" in i:continue  
                if "_OEV" not in i:continue
                a100 = pandas.DataFrame(np.array(gr_anb[i]))
                a100.loc[:,"n"] = 1
                gr_v = f["Ergebnisse_OEV"]
                data_v = pandas.DataFrame(np.array(gr_v[i.replace("100",Bezug[1])])) ##name Vergleichssystem                        
                if Bezug[0] == "ID_StatGeb":a100 = a100[a100["ID_StatGeb"]!=9999] ##no missing Stat Areas
                if Bezug[0] == "ID_Gemeinde":a100 = a100[a100["ID_Gemeinde"]!=9999] ##no missing Stat Areas            
                ...
                if "_OEV" in i:
                    ...
                    ##--export--##
#                    if "E_Hausarzt_OEV_100" == i and export_bezug == Bezug[0]: hh
                    if export == 1: 
                        if "E_Hausarzt_OEV_100" == i and export_bezug == Bezug[0]: zbezug.to_excel("U:"+f[2]+export_bezug.split("_")[1]+"_VarK_"+zent+"_OEV.xls")
        ...
        if "Anbindung_OEV" in groups:
            ...
            for i in gr_anb:
                ...
                a100.loc[:,"n"] = 1
                gr_v = f["Ergebnisse_OEV"]
                v_name = i.replace("100",Bezug[1])
                v_name = v_name.replace("gew_1","gew")
                try:data_v = pandas.DataFrame(np.array(gr_v[v_name])) ##name Vergleichssystem
                except:   data_v = pandas.DataFrame(np.array(gr_v[v_name[:-2]]))                      
                if Bezug[0] == "ID_StatGeb":a100 = a100[a100["ID_StatGeb"]!=9999] ##no missing Stat Areas
                if Bezug[0] == "ID_Gemeinde":a100 = a100[a100["ID_Gemeinde"]!=9999] ##no missing Stat Areas            
